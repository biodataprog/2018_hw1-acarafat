...
echo 'First 25 lines: ' `head -25 *BLASTP.tab`
echo 'Last 3 lines: ' `tail -3 *BLASTP.tab`
echo  'Total lines: ' `wc -l *BLASTP.tab`
...
echo '# of codons for each amino acids:' `cat codon_table.txt | awk '{print $3}' | sort | uniq -c`
